...
(function($){
	...
	var Redactor = function(element, options)
	{
		...
		this.opts = $.extend({
			...
			buttonsCustom: {
                fontfamily: {
                    ...
                    dropdown: {
                 	...
                 	Authman: {title: 'خط عثمان طه',callback: function(obj){obj.insertHtml('<span class=uthman >'+obj.getSelectedHtml()+'</span>');}}, 
                 	AuthmanBold: {title: 'خط عثمان طه-غامق',callback: function(obj){obj.insertHtml('<span class=uthmanb >'+obj.getSelectedHtml()+'</span>');}} , 
                 	Tohama: {title: 'عادي',callback: function(obj){obj.insertHtml('<span class=fonttohama >'+obj.getSelectedHtml()+'</span>');}}
                    }
                } , 
                ...
                fontsize: {
                    title: 'حجم الخط',
                    dropdown: {
                    	XXLarge: {title: 'XX-Large',callback: function(obj){obj.insertHtml('<span class=fontxxlarge >'+obj.getSelectedHtml()+'</span>');}},
                    	XLarge: {title: 'X-Large',callback: function(obj){obj.insertHtml('<span class=fontxlarge >'+obj.getSelectedHtml()+'</span>');}},
                    	Large: {title: 'Large',callback: function(obj){obj.insertHtml('<span class=fontlarge >'+obj.getSelectedHtml()+'</span>');}} ,
                    	Small: {title: 'Small',callback: function(obj){obj.insertHtml('<span class=fontsmall >'+obj.getSelectedHtml()+'</span>');}}
                    }
                } ,
                ...
                tools: {
                    title: 'أدوات',
                    dropdown: {
                    	CleanStyle: { title: 'clean',callback: function(obj){
                    		var text =obj.getSelectedHtml() ;
                    		$.ajax({
                    			url: "/site/clearStyle",
                    			type:'POST',
                    			data: 'text='+text,
                    			success: function(cleanText){
                    				obj.insertHtml('<span class=fontxxlarge >'+cleanText+'</span>');
                    			}
                    		});  
			...
			// private
			emptyHtml: '<span class=maqola_body> <br/> <br/ >  </span>',
			buffer: false,
			visual: true,
	...
	Redactor.prototype = {
		...
		build: function(mobile, whendone)
		{
			if (mobile !== true)
			{
				...
				if (this.$editor)
				{
					this.$editor.addClass('redactor_editor').attr('contenteditable', true).attr('dir', this.opts.direction).css( 'height' , '200px'  );
					this.$editor.addClass('maqola_body');
				}
